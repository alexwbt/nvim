...
- `<leader>F` (normal) formats with `lsp_fallback = true`, `async = true`. For Java buffers this is extended (buffer-local in `lua/config/lsp/jdtls.lua`) to run jdtls's `java/organizeImports` request first, then format.
- `formatters_by_ft` (in `lua/config/conform.lua`): `prettier` for js/ts/jsx/tsx/json/jsonc/html/css/scss/yaml/markdown; `shfmt` for sh/bash/zsh; `clang-format` for c/cpp. Binaries must be on PATH (node/prettier, shfmt, clang-format) via the MSYS2 shell PATH. For C/C++, `<leader>F` runs `clang-format` via conform (not clangd's built-in LSP formatter); `lsp_fallback` only fires if `clang-format` fails.
