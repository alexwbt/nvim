...
- Snacks.nvim (`lazy = false`, `priority = 1000`): `dashboard` + `input` + `picker` are enabled in `lua/config/snacks.lua`. Dashboard opened via `<leader>;`. Its `config =` callback is where `config.snacks` gets required (see the load-order note above).
